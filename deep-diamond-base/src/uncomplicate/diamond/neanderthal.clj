;;   Copyright (c) Dragan Djuric. All rights reserved. use and distribution terms for this software are covered by the
;;   Eclipse Public License 1.0 (http://opensource.org/licenses/eclipse-1.0.php) or later
;;   which can be found in the file LICENSE at the root of this distribution.
;;   By using this software in any fashion, you are agreeing to be bound by
;;   the terms of this license.
;;   You must not remove this notice, or any other, from this software.

(ns ^{:author "Dragan Djuric"}
    uncomplicate.diamond.neanderthal
  "TODO"
  (:require [uncomplicate.diamond.tensor :refer [*diamond-factory*]]
            [uncomplicate.diamond.internal.protocols :refer [neanderthal-factory diamond-factory]]
            [uncomplicate.diamond.internal.neanderthal.tensor :refer [neanderthal-tensor]]))

(defn tensor
  ([tz-factory shape type layout]
   (neanderthal-tensor (neanderthal-factory (diamond-factory tz-factory) type)
                       shape layout true))
  ([tz-factory shape type]
   (neanderthal-tensor (neanderthal-factory (diamond-factory tz-factory) type)
                       shape true))
  ([shape type]
   (if (keyword? type)
     (tensor *diamond-factory* shape type)
     (neanderthal-tensor type shape)))
  ([desc]
   (tensor *diamond-factory* desc type)))
