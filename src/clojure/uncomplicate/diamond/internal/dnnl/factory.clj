...
;;    `(do
;;       (check-contiguous ~x)
;;       (~method (.dim ~x) (.buffer ~x) (.offset ~x) 1)))
;;   ([method x y]
;;    `(do
;;       (check-contiguous ~x ~y)
;;       (~method (.dim ~x) (.buffer ~x) (.offset ~x) 1 (.buffer ~y) (.offset ~y) 1))))

;; (defmacro tensor-amax [iamax da x]
;;   `(if (< 0 (.dim ~x))
;;      (do
;;        (check-contiguous ~x)
;;        (Math/abs (.get ~da (.buffer ~x) (+ (.offset ~x) (tensor-method ~iamax ~x)))))
;;      0.0))

;; (defmacro tensor-laset [method alpha x]
;;   `(do
;;      (with-lapack-check
;;        (~method CBLAS/ORDER_ROW_MAJOR (int \g) (.dim ~x) 1 ~alpha ~alpha (.buffer ~x) (.offset ~x) 1))
;;      ~x))
...
;;    `(do
;;       (check-contiguous ~x ~y)
;;       (~method (.dim ~x) ~alpha (.buffer ~x) (.offset ~x) 1 (.buffer ~y) (.offset ~y) 1)
;;       ~y))
;;   ([method alpha x beta y]
;;    `(do
;;       (check-contiguous ~x ~y)
;;       (~method (.dim ~x) ~alpha (.buffer ~x) (.offset ~x) 1 ~beta (.buffer ~y) (.offset ~y) 1)
;;       ~y)))

;; (defmacro tensor-scal [method alpha x]
;;   `(do
;;      (check-contiguous ~x)
;;      (~method (.dim ~x) ~alpha (.buffer ~x) (.offset ~x) 1)
;;      ~x))
